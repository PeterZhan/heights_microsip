...
{
	//microsipDlg->ShowMessage(L"FindString");
	long m_AktCount = (long)m_DisplayList.GetSize();
	//microsipDlg->ShowMessage(L"FindString 1");
	if(!m_bDisplayOnly)
	{
	//	microsipDlg->ShowMessage(L"FindString 2");
	...
	//	microsipDlg->ShowMessage(L"FindString 3");
		m_DisplayList.RemoveAll();

		m_Str2.MakeUpper();

	//	microsipDlg->ShowMessage(m_Str2 + L"FindString 4");
		...
		for(int i = nStartAfter+1; i < m_SearchList.GetSize(); i++)
		{
			...
			m_Str1.MakeUpper();
		//	microsipDlg->ShowMessage( L"FindString 5");
			if(m_lMode & _MODE_FIND_ALL_)
			{
			//	microsipDlg->ShowMessage(L"_MODE_FIND_ALL_ 6");
				if(m_Str1.Find(m_Str2) >= 0)
				{
				//	microsipDlg->ShowMessage(L"_MODE_FIND_ALL_ 7");
					//microsipDlg->ShowMessage(m_SearchList.GetAt(i));
