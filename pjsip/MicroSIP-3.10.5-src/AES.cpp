...
boolean AES::Cipher(CString& input, CByteArray& output)
{
	CStringA Ainput(input);
	const char * cinput = (const char *)Ainput;

	unsigned char uch_input[1024];
	strToUChar(cinput, uch_input);

	Cipher(uch_input);

	char coutput[1024];

	ucharToHex(uch_input, coutput);
	...
	for (int i = 0; i < len; i++)
	{
		//output.SetAt(i, coutput[i]);
		output.Add(coutput[i]);
	...
	//output = *(new CByteArray(uch_input));
	//ucharToHex(uch_input, output);
}

boolean AES::InvCipher(CByteArray& input, CString& output)
{
	unsigned char uch_input[1024];
	char cinput[1024];
	...
	{
		cinput[i] = input.GetAt(i);
	}

	cinput[input.GetSize()] = 0;

	hexToUChar(cinput, uch_input);

	InvCipher(uch_input);

	ucharToStr(uch_input, coutput);

	//output.SetString()
	
	//output = new CString();
	for (int i = 0; i < strlen(coutput); i++){

		//output.SetAt(i, coutput[i]);
		output.AppendChar(coutput[i]);

	}
//	output.SetAt(strlen(coutput), 0);
