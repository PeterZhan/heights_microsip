...
BOOL Contacts::OnInitDialog()
{
	...
	list->InsertColumn(2, Translate(_T("type")), LVCFMT_LEFT, accountSettings.callsWidth2>0 ? accountSettings.callsWidth2 : 71);
...
#endif
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_CONTACTS, &Contacts::OnLvnItemchangedContacts)
END_MESSAGE_MAP()
...
void Contacts::OnMenuCallPickup()
{
	...
	if (pos) {
		int i = list->GetNextSelectedItem(pos);
		Contact *pContact = (Contact *) list->GetItemData(i);
		microsipDlg->messagesDlg->CallMake(pContact->number);
	}
}
...
void Contacts::OnMenuDelete()
{
	if (::MessageBox(
		this->m_hWnd,
		(LPCWSTR)L"Are you sure to delete the contacts selected?",
		(LPCWSTR)L"Delete Contacts",
		MB_OKCANCEL
		) != IDOK)
		return;
...
/*	if (::MessageBox(
		this->m_hWnd,
		(LPCWSTR)L"Are you sure to clear all contacts?",
		(LPCWSTR)L"Delete Contacts",
		MB_OKCANCEL
		) != IDOK)
		return;
...
bool Contacts::ContactAdd(CString number, CString name, CString type, char presence, char directory, BOOL save, BOOL fromDirectory)
{
	...
	if (fromDirectory || save) {
		...
		for (int i=0;i<count;i++) {
			...
			if (pContact->number == number) {
				...
				if (!type.IsEmpty() && type != pContact->type) {
					list->SetItemText(i, 2, type);
					pContact->type = type;
					changed = true;
				}
...
//	microsipDlg->ShowMessage(number + name);
	...
	CString tmpNumber = number;
	int p = number.Find(L"@");
	if (p != -1)
		tmpNumber = number.Mid(0, p);




	list->SetItemText(i, 1, tmpNumber);
	list->SetItemText(i, 2, type);
	if (save) {
		ContactsSave();
...
void Contacts::ContactsSave()
{
	...
	for (int i=0;i<count;i++) {
		...
		xmlAttr = new CXMLElement();
		xmlAttr->Create(_T("type"), XET_ATTRIBUTE);
		xmlAttr->SetValue(AnsiToUnicode(Utf8EncodeUcs2(XMLEntityEncode(pContact->type))).GetBuffer());
		xmlContact->AppendChild(xmlAttr);
	...
	if (xmlFile.LoadFromFile(filename.GetBuffer())) {
					...
					if (xmlContact->GetElementType() == XET_TAG) {
						...
						while (xmlAttr) {
							if (xmlAttr->GetElementType() == XET_ATTRIBUTE) {
								...
								else if (attrName == _T("type")) {
									type = XMLEntityDecode(Utf8DecodeUni(UnicodeToAnsi(xmlAttr->GetValue())));
								}
						...
						if (!number.IsEmpty()) {
							ContactAdd(number, name, type, presence, directory, FALSE);
						}
					}
		...
		while (TRUE) {
			...
			if (GetPrivateProfileString(_T("Contacts"), key, NULL, ptr, 256, accountSettings.iniFile)) {
				...
				BOOL presence;
				BOOL directory;
				ContactDecode(ptr, number, name, type, presence, directory);
				ContactAdd(number, name, type, presence, directory, FALSE);
			} else {
				break;
			}
...
void Contacts::ContactDecode(CString str, CString &number, CString &type, CString &name, BOOL &presence, BOOL &fromDirectory)
{
	CString rab;
	int begin;
	int end;
...
//	microsipDlg->ShowMessage(sipURI);
	int n = list->GetItemCount();
	for (int i=0; i<n; i++) {
		Contact* pContact = (Contact *) list->GetItemData(i);
		CString conNum = pContact->number;
		int p = conNum.Find(L"@");
		

		if (p != -1)
		{
			conNum = conNum.Mid(0, p);
		}
	...
	//	microsipDlg->ShowMessage(pContact->number);
	//	microsipDlg->ShowMessage(conNum);
	//	microsipDlg->ShowMessage(number);
...
void Contacts::OnLvnItemchangedContacts(NMHDR *pNMHDR, LRESULT *pResult)
{
