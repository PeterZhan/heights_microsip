...
BOOL SettingsDlg::OnInitDialog()
{
  ...
  #ifdef _GLOBAL_MULTI_MODE
	GetDlgItem(IDC_SINGLE_MODE)->ShowWindow(SW_HIDE);
  #endif
	...
	((CButton*)GetDlgItem(IDC_STUNCHECK))->SetCheck(accountSettings.stuncheck);
	
  #ifndef _GLOBAL_FORCE_INCODEC
	...
	combobox = (CComboBox*)GetDlgItem(IDC_MICROPHONE_1);
	combobox->AddString(Translate(_T("Default")));
	combobox->SetCurSel(0);
	...
	for (unsigned i = 0; i<count; i++)
	{
		if (aud_dev_info[i].input_count) {
			CString audDevName(aud_dev_info[i].name);
			combobox->AddString(audDevName);
			if (!accountSettings.audioInputDevice_1.Compare(audDevName))
			{
				combobox->SetCurSel(combobox->GetCount() - 1);
			}
		}
	...
	combobox = (CComboBox*)GetDlgItem(IDC_SPEAKERS_1);
	combobox->AddString(Translate(_T("Default")));
	combobox->SetCurSel(0);
	...
	for (unsigned i = 0; i<count; i++)
	{
		if (aud_dev_info[i].output_count) {
			...
			if (!accountSettings.audioOutputDevice_1.Compare(audDevName))
			{
				combobox->SetCurSel(combobox->GetCount() - 1);
			}
...
void SettingsDlg::OnBnClickedOk()
{
	...
	accountSettings.stuncheck=((CButton*)GetDlgItem(IDC_STUNCHECK))->GetCheck();

  #ifndef _GLOBAL_FORCE_INCODEC
	accountSettings.forceCodec =  ((CButton*)GetDlgItem(IDC_FORCE_CODEC))->GetCheck();
  #else
	accountSettings.forceCodec = _GLOBAL_FORCE_INCODEC;
  #endif
#endif
	...
	GetDlgItem(IDC_MICROPHONE_1)->GetWindowText(accountSettings.audioInputDevice_1);
	if (accountSettings.audioInputDevice_1 == Translate(_T("Default")))
	{
		accountSettings.audioInputDevice_1 = _T("");
	}
	...
	GetDlgItem(IDC_SPEAKERS_1)->GetWindowText(accountSettings.audioOutputDevice_1);
	if (accountSettings.audioOutputDevice_1 == Translate(_T("Default")))
	{
		accountSettings.audioOutputDevice_1 = _T("");
	}
...
#endif
	if (accountSettings.stuncheck)
		accountSettings.account.stun =  _T(STUN_SERVER);
	else
		accountSettings.account.stun =  _T("");
	...
	if (accountSettings.denyIncoming == _T("all"))
		((CButton*)microsipDlg->pageDialer->GetDlgItem(IDC_DONTDIST))->SetIcon(microsipDlg->pageDialer->m_hIconDonot);
	else
		((CButton*)microsipDlg->pageDialer->GetDlgItem(IDC_DONTDIST))->SetIcon(microsipDlg->pageDialer->m_hIconIdle);
...
{
	GetDlgItem(IDC_CUSTOM_INI)->SetWindowText(NULL);
}
#endif
