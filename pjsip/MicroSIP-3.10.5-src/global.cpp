...
BOOL SelectSIPAccount(CString number, pjsua_acc_id &acc_id, pj_str_t &pj_uri)
{
	...
	BOOL isLocal = !accountSettings.accountId || accountSettings.account.domain.IsEmpty();
	if (!isLocal) {
	...
	acc_id = !isLocal && pjsua_acc_is_valid(account) ? account : account_local;
#ifdef _GLOBAL_NUMBER_PREFIX
	number = _T(_GLOBAL_NUMBER_PREFIX) + number;
#endif
	pj_uri = StrToPjStr ( GetSIPURI(number, accountSettings.accountId && accountSettings.account.domain!=sipuri.domain && !accountSettings.account.domain.IsEmpty(), isLocal) );
	return isLocal;
}
